...
# LICENSE https://github.com/zijundeng/pytorch-semantic-segmentation/blob/master/LICENSE

# 2D segmentation Network based on https://arxiv.org/pdf/1703.02719.pdf
...
from .config import res152_path   # Provided paths to pretrained net
...
class GCN(nn.Module):
    ...
    def forward(self, x):
        ...
        fs1 = self.brm5(F.interpolate(gcfm1, fm3.size()[2:], mode='bilinear', align_corners=False) + gcfm2)  # 32
        fs2 = self.brm6(F.interpolate(fs1, fm2.size()[2:], mode='bilinear', align_corners=False) + gcfm3)  # 64
        fs3 = self.brm7(F.interpolate(fs2, fm1.size()[2:], mode='bilinear', align_corners=False) + gcfm4)  # 128
        fs4 = self.brm8(F.interpolate(fs3, fm0.size()[2:], mode='bilinear', align_corners=False))  # 256
        out = self.brm9(F.interpolate(fs4, self.input_size, mode='bilinear', align_corners=False))  # 512
